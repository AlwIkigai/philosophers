...
/* Simulation timeout in milliseconds (default: 60 seconds) */
#define DEFAULT_SIMULATION_TIMEOUT 60000
...
typedef struct s_table
{
	...
	int             time_to_die;      // Time in ms before a philosopher dies without eating
	...
	long            timeout_duration; // Duration after which simulation times out (ms)
} t_table;
...
// debugger
void debug_log_locked(t_table *table, pthread_mutex_t *print_lock, const char *color, const char *format, ...);
...
void	signal_timeout(t_table *table, long now);
